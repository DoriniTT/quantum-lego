#!/usr/bin/env python
"""Plot band structure + (T)DOS/(P)DOS from a finished WorkGraph.
...
from examples._shared.config import setup_profile  # noqa: E402


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('workgraph_pk', type=int, help='PK of the WorkGraph node')
    parser.add_argument('--profile', default=None, help='AiiDA profile to load (default: QUANTUM_LEGO_PROFILE or AiiDA default)')
    parser.add_argument('--output', default=None, help='Output file path (default: bands_dos_<PK>.pdf)')
    parser.add_argument('--reference', choices=('fermi', 'vbm'), default='fermi', help='Energy reference for E=0 (default: fermi)')
    parser.add_argument(
        '--y-range',
        nargs=2,
        type=float,
...
def _get_vbm_from_bands(bands: BandsData) -> float | None:
...
def main() -> None:
    ...
    # Default output paths for the example workflow.
    try:
    ...
    tdos, element_dos, dos_efermi, _is_spin_dos = _get_dos_from_vasprun(dos_retrieved)
    ...
    gs = gridspec.GridSpec(1, 2, width_ratios=[2.0, 1.3], wspace=0.05)
    ...
    # DOS x-limits: scale to visible energy window only
    ...
    for curve in curves:
        if curve.size == 0:
            continue
        cur = curve[mask] if mask.any() else curve
        if cur.size:
            max_dos = max(max_dos, float(np.max(cur)))
