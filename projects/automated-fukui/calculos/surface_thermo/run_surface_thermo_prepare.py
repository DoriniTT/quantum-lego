#!/usr/bin/env python
"""SnO2 surface-thermo prep (single WorkGraph).

Workflow:
  1) Relax bulk SnO2 (vasp brick)
  2) Relax reference phase: Sn (bulk) (vasp brick)
  3) Compute O2 reference energy via water-splitting (H2 + H2O) (o2_reference_energy brick)
  4) Generate all symmetrized SnO2(110) slab terminations (surface_terminations brick)
  5) Relax all terminations in parallel (dynamic_batch brick)
  6) Compute bulk formation enthalpy ΔHf(SnO2) from Sn and O references (formation_enthalpy brick)
  7) Compute surface Gibbs free energies for each termination (surface_gibbs_energy brick)

This script does NOT yet connect to surface_enumeration (that can come later).

Usage:
    python run_surface_thermo_prepare.py
"""
...
# --- Structures ---
bulk_sno2 = orm.StructureData(ase=read(STRUCT_DIR / 'sno2.vasp'))
...
stages = [
    ...
    # Bulk formation enthalpy ΔHf(SnO2)
    {
        'name': 'dhf',
    ...
    # Surface Gibbs free energies for each termination (ab initio thermodynamics)
    {
        'name': 'surface_gibbs',
...
if __name__ == '__main__':
    result = quick_vasp_sequential(
        ...
        potential_mapping={'Sn': 'Sn_d', 'O': 'O', 'H': 'H'},
        options={
            'resources': {
                'num_machines': 1,
            ...
            'custom_scheduler_commands': (
                '#PBS -q par40\n'
                '#PBS -j oe\n'
                '#PBS -N sno2_surface_thermo\n'
            ),
        },
        max_concurrent_jobs=4,
        name='sno2_surface_thermo_prepare',
    )
    ...
    pk_file = Path(__file__).parent / 'surface_thermo_pks.txt'
    with open(pk_file, 'a') as f:
        timestamp = datetime.now().isoformat()
        f.write(f'{timestamp}  sno2_surface_thermo_prepare  PK={pk}\\n')
